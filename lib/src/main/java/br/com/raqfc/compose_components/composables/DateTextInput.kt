...
import br.com.justworks.volan2.common.presentation.UtilsView.Companion.getActivity
		...
		private fun dateTextInDate(dateText: String, isDateAndHour: Boolean): Date? {
			return when {
				...
				isDateAndHour -> {
					...
					if (sDate.isEmpty() || !validateDateInText(sDate[0])) {
						return null
					}
					...
					if (validateDateAndHourInText(dateText)) {
						...
						dateCompleteWithDefaultHour(sDate[0])
					}
				}
				!validateDateInText(dateText) -> {
					null
				}
				else -> {
					dateCompleteWithDefaultHour(dateText)
				}
			}
		}
				...
				TextField(
					...
					onValueChange = {
						if (it.isEmpty() && isFormRequired) {
							...
							isError =
								!(if (isDateAndHour) validateDateAndHourInText(it) else validateDateInText(
									it
								))
